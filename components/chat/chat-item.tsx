		...
		<div className='group relative flex w-full items-center p-4 transition hover:bg-black/5'>
			<div className='group flex w-full items-start gap-x-2'>
				...
				<div className='flex w-full flex-col'>
					...
					{!fileUrl && !isEditing && (
						<p className={cn('text-sm text-zinc-600 dark:text-zinc-300', deleted && 'mt-1 text-xs italic text-zinc-500 dark:text-zinc-400')}>
							{content}
							{isUpdated && !deleted && <span className='mx-2 text-[10px] text-zinc-500 dark:text-zinc-400'>(изменено)</span>}
						</p>
					)}
					{!fileUrl && isEditing && (
						<Form {...form}>
							...
							<span className='mt-1 text-[10px] text-zinc-400'>Нажмите escape для отмены, enter для сохранения</span>
						</Form>
					)}
				</div>
			</div>
