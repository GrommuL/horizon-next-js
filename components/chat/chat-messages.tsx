	...
	if (isLoading) {
		return (
			<div className='flex flex-1 flex-col items-center justify-center'>
				...
				<p className='text-xs text-zinc-500 dark:text-zinc-400'>Загрузка сообщений...</p>
			</div>
		)
	}
	...
	if (status === 'error') {
		return (
			<div className='flex flex-1 flex-col items-center justify-center'>
				...
				<p className='text-xs text-zinc-500 dark:text-zinc-400'>Что-то пошло не так!</p>
			</div>
		)
	}
			...
			{hasNextPage && (
				<div className='flex justify-center'>
					{isFetchingNextPage ? (
						...
						>
							Загрузить предыдущее сообщение
						</button>
					)}
				</div>
			)}
